...
/*    BitString notInstr;
	BSTR_SetBits(&notInstr,"0010000000000000");
	COMP_LoadWord(&comp,0,notInstr);
    COMP_Display(comp);
   
    BitString Instr;
	BSTR_SetBits(&Instr,"000000000000101");
	COMP_LoadWord(&comp,1,Instr);
    COMP_Display(comp);
   */ 

    /* following add instruction is   ADD Dest: R3, Source: R4, R5 */
