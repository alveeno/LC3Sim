/* Pair 7: Duc Nguyen and Alvin Nguyen
 * This program was going to take in an array of C string and convert each C string
 * to an assembly language instruction/value to the memory address. It has ADD, NOT,
 * LD, BR, OUT, and HALT instructions.
 * We noted that our Branch instruction wasn't working properly, line 82  was supposed
 * to take the twos comp of the PCOFFSET, however when we do that, the console was
 * filled with random characters, so we removed it currently to at least be able to
 * see what's wrong with the rest of the code.
 */
